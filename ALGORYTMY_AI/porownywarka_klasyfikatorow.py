...
for ds_cnt, ds in enumerate(datasets):
    ...
    #printowanie zbiorów danych
    cm = plt.cm.RdBu
    cm_bright = ListedColormap(["#FF0000","#0000FF"])
    ax = plt.subplot(len(datasets),len(classifiesrs) + 1,i)

    if ds_cnt == 0:
        ax.set_title("Input data")

    #wykreśl wykres punktowy (scatter)
    #punkty treningowe
    ...
    for name,clf in zip(names,classifiesrs):
        ax = plt.subplot(len(datasets),len(classifiesrs)+1,i)
        ...
        #wykreślenie punktów treningowych
        ax.scatter(X_train[:,0],X_train[:,1],c=y_train, cmap=cm_bright,edgecolor="k")
        #wykreślenie punktów testowych
